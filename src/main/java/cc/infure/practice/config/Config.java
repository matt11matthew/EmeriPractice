package cc.infure.practice.config;
...
public class Config {
    ...
    public Config(String name) {
        this.configurationFile = new File(InfurePractice.getInstance().getDataFolder() + "/" + name + ".yml");
        if (!InfurePractice.getInstance().getDataFolder().exists()) {
            InfurePractice.getInstance().getDataFolder().mkdirs();
        }
        if (!this.configurationFile.exists()) {
            try {
                this.configurationFile.createNewFile();
                InputStream inputStream = InfurePractice.getInstance().getResource(name + ".yml");
                if (inputStream != null) {
